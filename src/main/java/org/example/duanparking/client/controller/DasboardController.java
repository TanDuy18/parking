    ...
    private void startWebcam() {
        Task<Void> task = new Task<>() {
            ...
            protected Void call() {
                ...
                while (!isCancelled() && cameraActive) {
                    ...
                    if (bImage != null) {
                        ...
                        if (!detected && checkInOut == 1) {
                            ...
                            if (now - lastDetectTime >= DETECT_INTERVAL) {
                                ...
                                new Thread(() -> {
                                    String plate = DetectPlate.detectFromCamera(bImage);

                                    if (plate != null && !plate.isEmpty()) {
                                        detected = true;
                                        Platform.runLater(() -> plateField1.setText(plate));
                                    }
                                }).start();
                            }
                        }
...
//                    priceField0.setText(formatVND(data.getHistory().getFee()));
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (webcam == null) {
            openNotificationScreen("Không tìm thấy camera");
            return;
        }
